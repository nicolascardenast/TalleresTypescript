...
function desplegarDatosSeries(series: Serie[]): void {
...
function desplegarPromedios(series: Serie[]): void {
    let tbodypromedios: HTMLElement = document.createElement("tBody");
    let promedio: number = 0;
    for(let serie of series) {promedio += serie.seasons;}
    let res = promedio / series.length;
    let trElement: HTMLElement = document.createElement("tr");
    trElement.innerHTML = `<td class = "promedio">Seasons Average: ${res}</td>`;
    tbodypromedios.appendChild(trElement);    
    tablaPromedios.appendChild(tbodypromedios);
}

export const tv = new TvSeries(series);
let tablaSeries: HTMLElement = document.getElementById("serie")!;
let tablaPromedios: HTMLElement = document.getElementById("statistic")!;

desplegarPromedios(tv.series);
desplegarDatosSeries(tv.series);